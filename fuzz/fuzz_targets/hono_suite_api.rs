...
fuzz_target!(|data: &[u8]| {
    ...
    let response = runtime
        .call_app_fetch(fuzz_request(data))
        .expect("hono-suite request trapped");
...
fn fuzz_request(data: &[u8]) -> Request {
    ...
    match selector {
        ...
        2 => request("GET", "/api/report", Vec::new()),
        3 => request("GET", "/api/session", Vec::new()),
        4 => request("GET", &format!("/api/crypto?value={}", lossy(body)), Vec::new()),
        5 => request("GET", &format!("/api/check?url={}", lossy(body)), Vec::new()),
        6 => request("GET", "/api/incidents", Vec::new()),
        7 => request("GET", "/api/audit", Vec::new()),
        8 => json_request("/api/incidents", body),
        _ => request("POST", "/api/incidents/fuzz/resolve", Vec::new()),
    }
}
