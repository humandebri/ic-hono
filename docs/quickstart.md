...
Durable Edge Runtime on ICP は Edge-compatible な Hono app を local bytecode 化し、bytecode だけを canister に upload して実行する。
...
`scripts/icp_local_smoke.sh` は上記に加えて format、unit test、package smoke、wasm import audit を実行する。各 example は upload 前に `ic-edge pack` で bytecode を再生成し、canister upload は bytecode chunk API を使う。`icp build` / `icp deploy` の stdout/stderr は既定で `/tmp/ic-edge-smoke-logs` に保存する。保存先は `IC_EDGE_SMOKE_LOG_DIR` で変更できる。
