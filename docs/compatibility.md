...
| --- | --- | --- | --- |
| `hono` | pass | `examples/hono-basic` の manifest 付き IIFE bundle を QuickJS で eval。basic route、JSON、params/query、CORS、Cache roundtrip が通過 | `scripts/icp_local_smoke.sh` で manifest upload canister smoke、direct update / Gateway / stable Cache が通過 |
...
| Feature | Status | Note |
| --- | --- | --- |
| `app.fetch()` | pass | manifest 付き IIFE bundle の `default.fetch` を QuickJS から呼ぶ |
...
| rollback | pass | `runtime_history()` と `rollback_runtime()` で bundle/env/manifest snapshot を復元 |
...
cargo run -p ic-edge-pack -- upload \
  examples/hono-basic/dist/app.bundle.js --module app
cargo run -p ic-edge-pack -- upload \
  examples/hono-basic/dist/app.bundle.js --module app \
