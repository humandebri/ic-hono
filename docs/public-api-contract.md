...
- Canister methods: `http_request`, `http_request_update`, `upload_bundle` (manifest なし raw upload を拒否), `begin_bundle_upload(module, total_bytes, manifest_json)`, `append_bundle_chunk`, `commit_bundle_upload`, `abort_bundle_upload`, `set_env`, `env_names`, `bundle_size`, `runtime_info`, `runtime_history`, `rollback_runtime`, `fetch_outcall`, `fetch_outcall_replicated`
